...
int main()
{
  ...
  std::cout << "Initial End-Effector Position: " << forwardKinematics(myRobot).p.transpose()
            << "\n";
  std::cout << "Initial End-Effector Rotation:\n"
            << forwardKinematics(myRobot).R.eulerAngles(0, 1, 2) * 180 / M_PI << "\n";
  ...
  for (size_t i = 0; i < targets.size(); ++i)
  {
    ...
    auto result = solveIK(myRobot, targets[i], 1000);
    ...
    std::cout << "End-Effector Position: "
              << forwardKinematics(myRobot).p.transpose() << "\n";
  ...
  std::vector<Transform> poseTargets;
  ...
  for (size_t i = 0; i < poseTargets.size(); ++i)
  {
    ...
    auto result = solveIK6D(myRobot, poseTargets[i], 500);
    ...
    auto Tfinal = forwardKinematics(myRobot);
  ...
  exportForwardKinematics(myRobot, "forward_kinematics.csv");
  ...
  Trajectory traj = generateLinearTrajectory(Ts, Tt, duration, steps, 0.2 * duration);

  // Print the generated waypoints
  for (size_t i = 0; i < traj.waypoints.size(); ++i)
  {
    const Waypoint& wp = traj.waypoints[i];
